...
export class ClinicPetRegisterComponent implements OnInit {
  ...
  addressCodeParams: any = '';
  profileCodeParams: any = '';
  agentCodeParams: any = '';
  ...
  constructor(
    ...
    this.route.params.subscribe( params => {
      if (params['profilecode']) {
        this.profileCodeParams = params['profilecode'];
      }

      if (params['addresscode']) {
        this.addressCodeParams = params['addresscode'];
      }
    ...
    if (this.addressCodeParams !== '' && this.profileCodeParams !== '') {
      this.actionParams = 'edit';
    } else if (this.addressCodeParams === '' && this.profileCodeParams !== '') {
  ...
  ngOnInit() {
    this.breadcrumbParam = {
      profilecode : this.profileCodeParams,
      agentcode : this.agentCodeParams
    };
...
LoadLookup() {
  ...
  CheckProfileCode () {
    ...
    if (this.authenticationToken != null && this.profileCodeParams !== '') {
      const authorization = 'Bearer ' + this.authenticationToken;
      const endpoint = Registration.Agent.Inquiry.OwnerProfileByOwnerCode;
      const newEndpoint = endpoint.url.replace('{owner_code}', this.profileCodeParams);
  ...
  CheckAddressCode() {
    const initialState = this.themeConfig.defaultSettings.dialogInitialStateSetting;
    const configModal = this.themeConfig.defaultSettings.dialogAlertSetting;
    if (this.authenticationToken != null && this.addressCodeParams !== '') {
      const authorization = 'Bearer ' + this.authenticationToken;
      const endpoint = Registration.MySelf.Inquiry.AnimalOwnerAddressById;
      let newEndpoint = endpoint.url.replace('{animal_address_code}', this.addressCodeParams);
      newEndpoint = newEndpoint.replace('{myself_code}', this.profileCodeParams);
      this.Api.callWithOutScope(newEndpoint, endpoint.method, {},  'Authorization', authorization).then((res) => {
        if (res.contentObj !== null) {
          this.actionParams = 'edit';
  ...
  LoadData(pages: any) {
    ...
    if (this.actionParams === 'edit') {
      if (this.authenticationToken != null) {
        ...
        newEndpoint = newEndpoint.replace('{myself_code}', this.profileCodeParams);
        newEndpoint = newEndpoint.replace('{animal_address_code}', this.addressCodeParams);
        this.Api.callWithOutScope(newEndpoint, endpoint.method, {},  'Authorization', authorization).then((response) => {
          const res = response;
      ...
      if (this.ContentForm.valid) {
        ...
        this.Api.callWithOutScope(endpoint.url, endpoint.method, contentParams,  'Authorization', authorization).then((response) => {
          ...
          if (res.contentObj !== null) {
            ...
            if (res.contentObj !== null) {
              this.actionParams = 'edit';
              this.addressCodeParams = res.contentObj.code;
  ...
  ActionEditContent(value: any) {
    ...
    if (this.addressCodeParams !== '') {
      ...
      this.addressCodeParams = initialState.moduleContent.contentObj.registrationAnimalAddressCode;
      this.profileCodeParams = initialState.moduleContent.contentObj.registrationAnimalOwnerCode;
  ...
  ManageAnimalInformation() {
    ...
    if (this.addressCodeParams !== '') {
      const authorization = 'Bearer ' + this.authenticationToken;
      const endpoint = Registration.MySelf.Inquiry.AnimalOwnerAddressById;
      let newEndpoint = endpoint.url.replace('{myself_code}', this.profileCodeParams);
      newEndpoint = newEndpoint.replace('{animal_address_code}', this.addressCodeParams);
      this.Api.callWithOutScope(newEndpoint, endpoint.method, {},  'Authorization', authorization).then((res) => {
        if ( res.contentObj != null) {
          initialState = this.themeConfig.defaultSettings.formInitialStateSetting;
