...
@Component({
  selector: 'app-dialog-animal-symptom-manage',
  templateUrl: './dialog.animal.symptom.manage.component.html',
  styleUrls: ['./dialog.animal.symptom.manage.component.scss'],
  encapsulation: ViewEncapsulation.None,
  providers: [CallApiService]
})
export class DialogAnimalSymptomManageComponent implements OnInit {
  ...
  LoadModalHeader() {
    switch (this.actionForm) {
      case 'add':
        this.titleModal = 'เพิ่มข้อมูลประเภทของสัตว์';
        break;
      case 'edit':
        this.titleModal = 'แก้ไขข้อมูลประเภทของสัตว์';
        this.ContentSelect(this.dataObj, false);
        break;
      case 'view':
        this.titleModal = 'แสดงข้อมูลประเภทของสัตว์';
        this.ContentSelect(this.dataObj, true);
        break;
      default:
        this.titleModal = 'จัดการข้อมูลประเภทของสัตว์';
    }
  }
  ...
  ContentSelect(contentObj: any, disabled: boolean) {
    if (this.authenticationToken != null) {
      ...
      const endpoint = Animal.Symptom.Inquiry.ById;
    ...
    if (this.authenticationToken != null) {
      ...
      if (this.ContentForm.valid) {
        ...
        if (this.actionForm === 'add') {
          endpoint = Animal.Symptom.Create;
        } else if (this.actionForm === 'edit') {
          endpoint = Animal.Symptom.Update;
        }
        ...
        initialState.description = 'กรุณากรอกข้อมูลประเภทของสัตว์ให้ครบ';
        const bsModalRefObj = this.ModalService.show(DialogAlertComponent, Object.assign({}, configModal , { initialState }));
        bsModalRefObj.content.action.subscribe(result => {
          if (result.status) {
            this.ContentReset();
