...
export class DialogProvinceManageComponent implements OnInit {
  ...
  LookupRegion(regionCode: any = '') {
    this.lookupRegionCode = [];
    if (this.authenticationToken != null) {
      const authorization = 'Bearer ' + this.authenticationToken;
      const endpoint = Utility.Region.Inquiry.ByList.ListActive;
      this.Api.callWithOutScope(endpoint.url, endpoint.method, {},  'Authorization', authorization).then((response) => {
        if (response != null) {
          response.forEach(element => {
            this.lookupRegionCode.push({ value: element.code, label: element.nameTH });
          });

          if (regionCode !== '') {
            this.ContentForm.controls['regionCode'].setValue(regionCode);
          }
        } else {
          this.lookupRegionCode = [];
        }
  ...
  ContentSelect(contentObj: any, disabled: boolean) {
    if (this.authenticationToken != null) {
      ...
      this.Api.callWithOutScope(newEndpoint, endpoint.method, {},  'Authorization', authorization).then((response) => {
        ...
        if (res.region != null) {
          this.ContentForm.controls['regionCode'].setValue(res.region.code);
          this.LookupRegion(res.region.code);
        }
