...
@Component({
  selector: 'app-dialog-sample-objective-manage',
  templateUrl: './dialog.sample.objective.manage.component.html',
  styleUrls: ['./dialog.sample.objective.manage.component.scss'],
  encapsulation: ViewEncapsulation.None,
  providers: [CallApiService]
})
export class DialogSampleObjectiveManageComponent implements OnInit {
  ...
  LoadModalHeader() {
    switch (this.actionForm) {
      case 'add':
        this.titleModal = 'เพิ่มข้อมูลวัตถุประสงค์การเก็บตัวอย่าง';
        break;
      case 'edit':
        this.titleModal = 'แก้ไขข้อมูลวัตถุประสงค์การเก็บตัวอย่าง';
        this.ContentSelect(this.dataObj, false);
        break;
      case 'view':
        this.titleModal = 'แสดงข้อมูลวัตถุประสงค์การเก็บตัวอย่าง';
        this.ContentSelect(this.dataObj, true);
        break;
      default:
        this.titleModal = 'จัดการข้อมูลวัตถุประสงค์การเก็บตัวอย่าง';
    }
  }
  ...
  ContentSelect(contentObj: any, disabled: boolean) {
    if (this.authenticationToken != null) {
      ...
      const endpoint = Sample.Objective.Inquiry.ById;
    ...
    if (this.authenticationToken != null) {
      ...
      if (this.ContentForm.valid) {
        ...
        if (this.actionForm === 'add') {
          endpoint = Sample.Objective.Create;
        } else if (this.actionForm === 'edit') {
          endpoint = Sample.Objective.Update;
        }
        ...
        initialState.description = 'กรุณากรอกข้อมูลวัตถุประสงค์การเก็บตัวอย่างให้ครบ';
        const bsModalRefObj = this.ModalService.show(DialogAlertComponent, Object.assign({}, configModal , { initialState }));
        bsModalRefObj.content.action.subscribe(result => {
          if (result.status) {
            this.ContentReset();
