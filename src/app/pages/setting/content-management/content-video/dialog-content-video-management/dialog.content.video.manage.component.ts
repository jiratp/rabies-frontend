...
export class DialogContentVideoManageComponent implements OnInit {
  ...
  LoadModalHeader() {
    switch (this.actionForm) {
      case 'add':
        this.titleModal = 'เพิ่มข้อมูลคลิปวีดีโอ';
        break;
      case 'edit':
        this.titleModal = 'แก้ไขข้อมูลคลิปวีดีโอ';
        this.ContentSelect(this.dataObj, false);
        break;
      case 'view':
        this.titleModal = 'แสดงข้อมูลคลิปวีดีโอ';
        this.ContentSelect(this.dataObj, true);
        break;
      default:
        this.titleModal = 'จัดการข้อมูลคลิปวีดีโอ';
    }
  }
  ...
  ContentSave() {
    if (this.authenticationToken != null) {
      ...
      if (this.ContentForm.valid) {
        ...
        initialState.description = 'กรุณากรอกข้อมูลคลิปวีดีโอให้ครบ';
        const bsModalRefObj = this.ModalService.show(DialogAlertComponent, Object.assign({}, configModal , { initialState }));
        bsModalRefObj.content.action.subscribe(result => {
          if (result.status) {
            this.ContentReset();
