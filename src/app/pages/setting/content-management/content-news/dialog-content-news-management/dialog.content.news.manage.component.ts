...
@Component({
  selector: 'app-dialog-content-news-manage',
  templateUrl: './dialog.content.news.manage.component.html',
  styleUrls: ['./dialog.content.news.manage.component.scss'],
  encapsulation: ViewEncapsulation.None,
  providers: [CallApiService]
})
export class DialogContentNewsManageComponent implements OnInit {
  ...
  LoadModalHeader() {
    switch (this.actionForm) {
      case 'add':
        this.titleModal = 'เพิ่มข้อมูลข่าวประชาสัมพันธ์';
        break;
      case 'edit':
        this.titleModal = 'แก้ไขข้อมูลข่าวประชาสัมพันธ์';
        this.ContentSelect(this.dataObj, false);
        break;
      case 'view':
        this.titleModal = 'แสดงข้อมูลข่าวประชาสัมพันธ์';
        this.ContentSelect(this.dataObj, true);
        break;
      default:
        this.titleModal = 'จัดการข้อมูลข่าวประชาสัมพันธ์';
    }
  }
  ...
  ContentSelect(contentObj: any, disabled: boolean) {
    if (this.authenticationToken != null) {
      ...
      const authorization = 'Bearer ' + this.authenticationToken;
      const endpoint = Content.News.Inquiry.ById;
    ...
    if (this.authenticationToken != null) {
      ...
      if (this.ContentForm.valid) {
        ...
        if (this.actionForm === 'add') {
          endpoint = Content.News.Create;
          newEndpoint = endpoint.url;
        } else if (this.actionForm === 'edit') {
          endpoint = Content.News.Update;
          newEndpoint = endpoint.url;
        }
        ...
        initialState.description = 'กรุณากรอกข้อมูลข่าวประชาสัมพันธ์ให้ครบ';
        const bsModalRefObj = this.ModalService.show(DialogAlertComponent, Object.assign({}, configModal , { initialState }));
        bsModalRefObj.content.action.subscribe(result => {
          if (result.status) {
            this.ContentReset();
