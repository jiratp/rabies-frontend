...
import { DialogDepartmentNameManageComponent } from './dialog-department-name-management/dialog.department.name.manage.component';

@Component({
  selector: 'app-department-name-manage',
  templateUrl: './department.name.manage.component.html',
  styleUrls: ['./department.name.manage.component.scss'],
  encapsulation: ViewEncapsulation.None,
  providers: [CallApiService]
})
export class DepartmentNameManageComponent implements OnInit {
  modalRef: BsModalRef;
  ...
  LoadData(pages: any, animalNameCode: any) {
    if (this.authenticationToken != null) {
      ...
      const endpoint = Department.Name.Inquiry.ByList.List;
      let newEndpoint = endpoint.url.replace('{page_number}', pages);
      newEndpoint = newEndpoint.replace('{animal_Name_code}', animalNameCode);
  ...
  ActionDeleteContent(dataContent: any) {
    if (this.authenticationToken != null) {
      ...
      bsModalRefObj.content.action.subscribe(result => {
        ...
        if (result.status) {
          const authorization = 'Bearer ' + this.authenticationToken;
          const endpoint = Department.Name.Delete;
